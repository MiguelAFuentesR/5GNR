...
load("Models\CNN_Modelo7.mat"); %Este primer modelo corresponde al generado con MATLAB, haciendo variaciones en los parámetros del canal
...
v = 50; %Velocidad del usuario en km/h
...
rx1 = 10*log10(abs(estChannelGridNN(:,3,:)));
rx2 = 10*log10(abs(estChannelGridNN(:,7,:)));
rx3 = 10*log10(abs(estChannelGridNN(:,10,:)));
rx4 = 10*log10(abs(estChannelGridPerfect(:,10,:)));
...
figure;
sgtitle(['Muestra de 1 ms con SNR: ' num2str(SNRdB) ' dB'])
subplot(1,3,1)
...
xlabel('Símbolos OFDM');
ylabel('Subportadora');
title('Estimación ideal')
% 
subplot(1,3,3)
