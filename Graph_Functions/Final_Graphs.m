...
Model = "Autoencoder";
Models = ["Autoencoder" "Lineal" "Practical"];
Vel = "30";
        ...
        for i = interval
            ...
            for modul= 1:length(Mod) %
                if Paths(i) == ("Channel_"+Path_Type+"_Mod_"+Mod(modul)+"_Vel_"+Vel+".mat")
                    ...
                    label = [label Model+" "+ velo + " km/h"];
                    sgtitle("Estimated " + Path_Type + " using " + Mod(modul) + " subcarrier modulation" )
                end
            end
...
%         disp('Resultados del canal '+datos.Channel+' con modulación '+datos.Modulation+' y velocidad de '+datos.User_Velocity+ 'km/h ')
% 
% 
% label = datos.models;
%     label{1,2} = 'Ideal';
